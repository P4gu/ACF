...
public class MoveObjct2D : MonoBehaviour
{
    ...
    [Range(0.1f, 2.0f)]
    public float ditanciaMinimaConector = 0.5f;
    ...
    void FixedUpdate()
    {
       if(!estaArrastando && !estaConectado) 
       {
            ...
            if(distancia < ditanciaMinimaConector)
            {
                _rigidbody2D.gravityScale = 0;
            ...
            if(distancia < 0.01f) 
            {
                ...
                transform.root.gameObject.layer = LayerMask.NameToLayer("C");
            }
       } 
    }
}
